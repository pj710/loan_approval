...
1. These attributes are **NOT** included as model training features
2. They are used only for **post-hoc fairness auditing** to detect disparate impact
3. A fair representation set of data is learned from raw data and used to train models to lower bias.

### Fairness Metrics to Monitor

1. **Demographic Parity Difference (DPD)**: Difference in approval rates between demographic groups
   - Target: |DPD| < 0.05 (5 percentage points)

2. **Equalized Odds Difference (EOD)**: Difference in true positive rates and false positive rates for each group.
   - Target: |EOD| < 0.05
